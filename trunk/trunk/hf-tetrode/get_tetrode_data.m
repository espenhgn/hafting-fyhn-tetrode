...
addParamValue(p, 'verbose', def_verbose, @isMyLogical)
addParamValue(p, 'export_csv', def_writecsv, @isMyLogical)
...
function out = isMyLogical(n)
%ISMYLOGICAL    checks if value is logical. Allows integers

out = 0;
if islogical(n)
    out=1;
% Parser makes integers into doubles, so check if the double is integer    
elseif (n-int8(n))==0
    out=1;
end
