...
for isess=1:nsess
    ...
    for icell=1:ncells
        ...
        % spike rates and durations are just numbers, so use arrays
        cells(icell,1).spike_rates = zeros(norients,...
            max(trials_pr_orientation));
        cells(icell,1).stim_durations = zeros(norients,...
            max(trials_pr_orientation));
        cells(icell,1).blank_durations = zeros(norients,...
            max(trials_pr_orientation));
        ...
        for itimeint=1:length(logtime)
            ...
            % Append timestamps, spike rates and duration of stimulus and
            % blank periods within stimulus and grey period to *cells*
            % structure 
            ...
            cells(icell).stim_durations(iorient, nspike_trains(iorient)) = ...
                tend-tstart;
            cells(icell).blank_durations(iorient, nspike_trains(iorient)) = ...
                tend_base-tstart_base;
